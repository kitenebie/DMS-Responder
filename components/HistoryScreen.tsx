...
import { HistoryFilter, HistoricalIncident } from '@/types';
...
interface HistoryScreenProps {
  onBack: () => void;
  onOpenChat: (incidentId: string) => Promise<void> | void;
  isDarkMode: boolean;
}
...
export const HistoryScreen: React.FC<HistoryScreenProps> = ({
  ...
  const withTimeout = useCallback(async (offset: number) => {
    return Promise.race([
  ...
  const incidentTypes = useMemo(() => {
    return [...new Set(incidents.map((incident) => incident.type))];
  }, [incidents]);
  ...
  const filteredIncidents = useMemo(() => {
    ...
    if (filter.type !== 'all') {
      result = result.filter((incident) => incident.type === filter.type);
    }
    ...
    <View
      ...
      <View style={styles.incidentHeader}>
        ...
        <View style={styles.incidentHeaderRight}>
          ...
          <TouchableOpacity
            onPress={() => void onOpenChat(String(incident.id))}
